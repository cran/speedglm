speedlm <- function(formula,data,weights=NULL,offset=NULL,sparse=NULL,
  ...
  set <- list(sparselim=.9,camp=.01,eigendec=TRUE,row.chunk=NULL,
              tol.solve=.Machine$double.eps,tol.values=1e-7,tol.vectors=1e-7, 
              method = "eigen")
  nmsC <- names(set)
  ...
  if (!is.null(weights))  
    rval <- speedlm.wfit(y,X,offset=offset,w=weights,camp=set$camp,
                         sparselim=set$sparselim,eigendec=set$eigendec,
                         row.chunk=set$row.chunk,tol.solve=set$tol.solve,
                         sparse=sparse,tol.values=set$tol.values,
                         tol.vectors=set$tol.vectors, method=set$method,
                         intercept=attr(tf,"intercept")) else 
    rval <- speedlm.fit(y,X,offset=offset,row.chunk=set$row.chunk,camp=set$camp,
                        sparselim=set$sparselim,eigendec=set$eigendec,
                        tol.solve=set$tol.solve,sparse=sparse,
                        tol.values=set$tol.values,tol.vectors=set$tol.vectors,
...
update.speedlm <- function(object, data, weights = NULL, offset = NULL,
    ...
    set <- list(sparselim = 0.9, camp = 0.01, eigendec = TRUE, 
        ...
        tol.vectors = 1e-07, method = "eigen")
    nmsC <- names(set)
    set[(namc <- names(set.default))] <- set.default
    if (length(noNms <- namc[!namc %in% nmsC]) > 0) 
    ...
    rval <- list(coefficients = coefficients, coef = coef, df.residual = dfr, 
        X1X = X1X, Xy = Xy, XW1 = XW1, SW = SW, yy = yy, A = as(A, 
            "matrix"), n.obs = object$n.obs + nrow(X), RSS = as.numeric(RSS), 
        rank = rank, ok = ok, nvar = nvar, weights = weights, 
        zero.w = zero.w + object$zero.w, pw = pw, XTX = as(XTX, 
            "matrix"), sparse = sparse, "intercept"=object$intercept)
                        ...
                        tol.values=1e-7,tol.vectors=1e-7, method="eigen",...)
{
  ...
  A <-  if (sparse) crossprod(X) else cp(X,,row.chunk)
  ris <- if (eigendec) control(A,,tol.values,tol.vectors,,method) else 
           list("XTX"=A,"rank"=nvar,"pivot"=1:nvar) 
   ris$XTX <- if (sparse) as(ris$XTX,"dgCMatrix") else  as(ris$XTX,"matrix")  
  y <- y - offset
  ...
  rval    <- list("coefficients"=coefficients,"coef"=coef,"df.residual"=dfr,
                  ...
                  "yy"=yy,"X1X"=X1X,"intercept"=intercept) 
  class(rval) <- "speedlm"
  rval 
}
...
speedlm.wfit <- function(y,X,w,intercept=FALSE, offset=NULL,row.chunk=NULL,
  ...
  X1X <- colSums(sqwX)
  XW1 <- crossprod(w,X)  
  A <-  if (sparse) crossprod(sqwX) else cp(sqwX,,row.chunk)      
  ris <- if (eigendec) control(A,,tol.values,tol.vectors,,method) else 
           list("XTX"=A,"rank"=nvar,"pivot"=1:nvar)
  ris$XTX <- if (sparse) as(ris$XTX,"dgCMatrix") else  as(ris$XTX,"matrix")  
  y <- y - offset
  Xy <- if (sparse) crossprod(X,(w*y)) else t(crossprod((w*y),X)) 
  ok <- ris$pivot[1:ris$rank]
  coef <- as(solve(ris$XTX,Xy[ok]),"numeric")
  coefficients <- rep(NA,nvar)
  coefficients[ok] <- coef
