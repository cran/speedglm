                          ...
                          method = "eigen", sparse=NULL,...)
{
  ...
  names(coefficients) <- variable
  rval <- list("coefficients"=coefficients,"logLik"=ll.nuovo,"iter"=iter,
               "tol"= tol,"family"=fam,"link"=link,"df"=dfr,"XTX"=XTX,
               "dispersion"=dispersion,"ok"=ok,"rank"=rank,"RSS"=RSS,
        ...
        while (!eof) {
            ...
            if (iter > 1) {
                if (is.null(sp))
                  sparse <- is.sparse(X = X, set$sparselim, set$camp)
                if (nvar == 1) {
                  eta <- offset + as.vector(X * start)
                } else {
                  eta <- offset + if (sparse)
                    drop(X[, ok] %*% start) else drop(tcrossprod(X[, ok], t(start)))
                }
                mu <- linkinv(eta <- eta)
            } else {            
                if (is.null(mustart)) eval(family$initialize)
                if (is.null(start)) {
                  eta <- if (is.null(etastart)) linkfun(mustart) else etastart
                  mu <- mustart
                } else {
                  if (nvar == 1) {
                    eta <- offset + as.vector(X * start)
                    ...
                    eta <- offset + if (sparse)
                      drop(X %*% start) else as.vector(tcrossprod(X, t(start)))
                  }
                  mu <- linkinv(eta)
                }
            }
